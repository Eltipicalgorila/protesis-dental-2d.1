...
public class Letras : MonoBehaviour
{
    ...
    IEnumerator SecuenciaTexto()
    {
        ...
        {
            t4 = "Entre todas las personas que pasaron, reconoces al hermano de un amigo tuyo, te alegras bastante de que este vivo y haya podido pasar la frontera. Los soldados bajan la guardia y empiezan a hablar entre ellos, desde la ventana escuchas que hablan sobre algunos datos que esconde el gobierno sobre los informes de los dentistas de las personas ¿Qué tendra que ver eso con los rumores de los monstruos";
        }
        else if (Humanos == 0 && Monstruos > 0)
        {
            t4 = "Durante el día no se ha registrado ningun monstruo, Feliz día del reves. Uno de los soldados desapareció de la guardia y el otro dijo que lo último que vio fue su zapado ensangrentado. ¡¿Qué le esta pasando al mundo?!";
        }
        else if (Humanos > 0 && Humanos < 3 && Monstruos > 0)
        {
            t4 = "Durante el día se ha notificado personas con aspecto monstruoso atacando gente y soldados,entre ellos, uno de tus soldados quedó abatido, por lo que cuentas solo con uno para la guardia, no sabes porque pero, sientes que parte de esto es por tu culpa";
        }
        else if (Humanos == 3 && Monstruos > 0)
        {
